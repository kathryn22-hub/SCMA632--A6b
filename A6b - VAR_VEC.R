...
perform_adf_test <- function(column_data, column_name) {
  tryCatch({
    adf_result <- ur.df(column_data, type = "none", selectlags = "AIC")
    p_value <- adf_result@testreg$coefficients[2, 4]  # Extract p-value for the test
    cat("\nADF test result for column:", column_name, "\n")
    print(summary(adf_result))
    return(p_value)
  }, error = function(e) {
    cat("\nError performing ADF test on column:", column_name, "\n")
    print(e)
    return(NA)
  })
}
